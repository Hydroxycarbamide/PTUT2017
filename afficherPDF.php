...
function Footer()
{
    ...
    $this->Cell(0,10,utf8_decode("Creer par glassite  ").$this->PageNo().'/{nb}',0,0,'C');
}
...
$pdf->putLink('http://intra.info.iut-tlse3.fr/~ptutc1/afficherPDF.php#page=4','Présentation du congrès 2018 ');
//saut de ligne
$pdf->Ln(5);
...
foreach($datesA as $D){
	...
	foreach($ateliers as $A){
		...
		//titre de l'atelier
		$pdf->Ln(1);
		$pdf->MultiCell(0,5,$pdf->putLink('http://intra.info.iut-tlse3.fr/~ptutc1/afficherPDF.php#page=6',"$A[4]"));
		}
	}
}
//saut de ligne
...
foreach($datesA as $D){
	...
	foreach($conference as $A){
		...
		$pdf->Ln(1);
		$pdf->MultiCell(0,5,$pdf->putLink('http://intra.info.iut-tlse3.fr/~ptutc1/afficherPDF.php#page=13',"$A[4]"));
		}
	}
}
